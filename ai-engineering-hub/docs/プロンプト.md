...
microcms_create_content({
  ...
  content: {
    ...
    category: "-9XEEE9fWf",  // 🚨 カテゴリーはIDを文字列で指定
    tags: "Claude,API,開発",  // 🚨 タグは文字列（カンマ区切り）
    contentType: "experience",  // 🚨 camelCase、文字列型
    targetAudience: "engineer",  // 🚨 camelCase、文字列型
    difficultyLevel: "intermediate",  // 🚨 camelCase、文字列型
    reading_time: 5,  // 🚨 数値型
...
### タグ（文字列で指定、カンマ区切り）
```
"Claude,API,開発"     ← Claude関連記事
"GPT,OpenAI,AI"      ← GPT関連記事
"Dify,ワークフロー"    ← Dify関連記事
"プロンプト,エンジニアリング" ← プロンプトエンジニアリング
"業務自動化,効率化"     ← 業務自動化
"API,開発,インテグレーション" ← API関連
"Next.js,React,フロントエンド" ← Next.js関連
"TypeScript,型安全"    ← TypeScript関連
```

### contentType（文字列で指定）
```
"experience"  ← 実際に試した体験談
"research"    ← 技術調査・比較分析
"tutorial"    ← 手順解説・学習ガイド
```

### targetAudience（文字列で指定）
```
"engineer"    ← エンジニア向け
"enterprise"  ← 企業向け
"both"        ← 両方
...
// ❌ 配列フィールドが文字列（必ずエラー）
content_type: "実体験"
target_audience: "ymdHpq2hwl"
difficulty_level: "nK9j4HWBVq"

// ❌ publishedAt がない（必ずエラー）
// publishedAt フィールドが欠けている
...
□ category は文字列（"-9XEEE9fWf"形式）
□ tags は配列（["dV2zRDXUm-"]形式）
□ content_type は配列（["実体験"]形式）
□ target_audience は配列（["ymdHpq2hwl"]形式）
□ difficulty_level は配列（["nK9j4HWBVq"]形式）
□ reading_time は数値型
□ publishedAt がISO 8601形式で記入済み
...
3. "contentType は文字列になっている"
4. "targetAudience は文字列になっている"
5. "difficultyLevel は文字列になっている"
